...
<div class="container mb-32 mx-auto px-10">
  ...
  <section class="text-center tracking-widest">
    <Title label="入部フォーム" />
    ...
    <form @submit.prevent="submitForm" action="">
      <div class="max-w-xs mx-auto flex-col">
        <div class="my-4">
          ...
          <input id="student-no" v-model="form.studentNumber" type="text" minlength="8" maxlength="8" name="entry.1552183669" required placeholder="01234567" class="inputbox" :class="{'invalid': !valid.studentNumber}" @change="valid.studentNumber = true">
        </div>
        <div class="my-4">
          <label for="real-name" class="text-left block">名前*</label>
          <input id="real-name" v-model="form.realName" type="text" name="entry.514745000" required placeholder="電算 太郎" class="inputbox">
        </div>
        <div class="my-4">
          <label for="email" class="text-left block">大学のメールアドレス（半角）*</label>
          <input id="email" v-model="form.email" type="email" name="entry.821989733" required placeholder="example@okayama-u.ac.jp" class="inputbox" :class="{'invalid': !valid.email}" @change="valid.email = true">
        </div>
        <div class="my-4">
          <label for="phone" class="text-left block">電話番号（半角）*</label>
          <input id="phone" v-model="form.phone" type="tel" minlength="9" maxlength="13" name="entry.793184820" required placeholder="090-1234-5678" class="inputbox" :class="{'invalid': !valid.phone}" @change="valid.phone = true">
        </div>
        <div class="my-4">
          <label for="nickname" class="text-left block">ニックネーム</label>
          <input id="nickname" v-model="form.nickname" type="text" name="entry.1050056731" placeholder="任意" class="inputbox">
        </div>
        <div class="my-4">
          <label for="hobby" class="text-left block">趣味</label>
          <input id="hobby" v-model="form.hobby" type="text" name="entry.528663940" placeholder="電子工作,アプリ開発,ゲーム,etc..." class="inputbox">
        </div>
        <div class="my-4">
          <label for="comment" class="text-left block">一言コメント</label>
          <input id="comment" v-model="form.comment" type="text" name="entry.1197263570" placeholder="任意" class="inputbox">
        </div>
      </div>
...
<style scoped>
.inputbox {
  @apply border-b border-black border-solid leading-10 text-center w-full tracking-widest
}
